...
(defmethod inspect* :string [truncate ^String x]
  (let [n (if truncate 5 20)
        suffix (if (> (.Length x) n) "...\"" "\"")]                         ;;; .length
    (str
      \"
      (.Substring ^String x 0 (min n (.Length x)))                          ;;; .substring  .length
      suffix)))

(defmethod inspect* :strable [truncate x] (str x))

(defmethod inspect* clojure.lang.PersistentVector+ChunkedSeq [truncate x]   ;;; clojure.lang.PersistentVector$ChunkedSeq
  "<vec seq>")

(defmethod inspect* clojure.lang.PersistentArrayMap+Seq [truncate x]        ;;; clojure.lang.PersistentArrayMap$Seq
  "<map seq>")

(defmethod inspect* clojure.lang.PersistentHashMap+NodeSeq [truncate x]     ;;; clojure.lang.PersistentHashMap$NodeSeq 
  "<map seq>")
...
(defmethod inspect* :default [truncate x]
  (let [classname (if (nil? x) "nil" (.Name (class x)))]                     ;;; .getName
    (str "<" classname ">")))
